function calculateDodecahedronVertices(size: number): number[] {
    // Formula for the radius of each face
    const r = Math.sqrt(3) / 2 * size;
    // Create an array of vertices for a dodecahedron
    return [
        r, 0, 0, // 1
        -r, 0, 0, // 2
        0, r, 0, // 3
...
function calculateIcosahedronVertices(size: number): number[] {
...
const dodecahedronVertices = calculateDodecahedronVertices(1);
console.log('Dodecahedron Vertices:', dodecahedronVertices);

const icosahedronVertices = calculateIcosahedronVertices(1);
